...
/**
 * @file kcipher2.c
 * @brief KCipher-2 stream cipher -- host-side implementation (opaque ADT).
 *
 * Contains the complete KCipher-2 algorithm: S-box, GF(2^8) helpers,
 * NLF, LFSR feedback polynomials (amul0..amul3), key expansion, init-mode
 * and normal-mode state advance, and the keystream generation loop.
 *
 * The struct KCipher2 definition is private to this file; external code
 * accesses it only via the pointer-based API declared in kcipher2.h.
 *
 * Reference: ISO/IEC 18033-4; K. Kiyomoto et al., KDDI R&D, 2007.
 *
 */
...
/* =========================================================================
 *  AES S-box (shared with KCipher-2's SubK2 transform)
 * ========================================================================= */
...
/* =========================================================================
 *  LFSR feedback polynomial lookup tables (amul0 .. amul3)
 *
 *  Each table encodes one of the four GF(2^32) feedback multiplications
 *  used in the KCipher-2 LFSR update step.
 * ========================================================================= */
...
/* =========================================================================
 *  Internal KCipher-2 state (hidden from callers)
 * ========================================================================= */

struct KCipher2 {
    uint32_t a[5];     ///< LFSR register A (5 words).
    uint32_t b[11];    ///< LFSR register B (11 words).
    uint32_t l1;       ///< FSM register L1.
    uint32_t r1;       ///< FSM register R1.
    uint32_t l2;       ///< FSM register L2.
    uint32_t r2;       ///< FSM register R2.
    uint8_t sbytes[8]; ///< Buffered keystream bytes (partial block).
    int svalid;        ///< Number of valid bytes remaining in sbytes.
};

/* =========================================================================
 *  GF(2^8) helpers and SubK2
 * ========================================================================= */

/**
 * @brief Non-linear function combining four 32-bit words.
 */
...
/**
 * @brief GF(2^8) multiplication by 2, with reduction by 0x11B and XOR 0xFF.
 */
static inline uint32_t gf_mult_by_2(uint8_t t) {
...
/**
 * @brief GF(2^8) multiplication by 3, with reduction by 0x11B and XOR 0xFF.
 */
static inline uint32_t gf_mult_by_3(uint8_t t) {
    uint32_t lq = ((uint32_t)t << 1) ^ (uint32_t)t;
    if (lq & 0x100)
        lq ^= 0x011B;
    return (lq ^ 0xFF);
}
...
/**
 * @brief SubK2 transform: S-box substitution + MDS matrix multiplication.
 *
 * Applies the AES S-box to each byte, then multiplies by the KCipher-2
 * MDS matrix in GF(2^8).
 *
 * @param in  Input 32-bit word.
 * @return    Transformed 32-bit word.
 */
static inline uint32_t sub_k2(uint32_t in) {
    uint8_t t0 = s_box[in & 0xff];
    uint8_t t1 = s_box[(in >> 8) & 0xff];
    uint8_t t2 = s_box[(in >> 16) & 0xff];
    uint8_t t3 = s_box[(in >> 24) & 0xff];
    uint32_t q0 = gf_mult_by_2(t0) ^ gf_mult_by_3(t1) ^ t2 ^ t3;
    uint32_t q1 = t0 ^ gf_mult_by_2(t1) ^ gf_mult_by_3(t2) ^ t3;
    uint32_t q2 = t0 ^ t1 ^ gf_mult_by_2(t2) ^ gf_mult_by_3(t3);
    uint32_t q3 = gf_mult_by_3(t0) ^ t1 ^ t2 ^ gf_mult_by_2(t3);
    return (q3 << 24) | (q2 << 16) | (q1 << 8) | q0;
}

/* =========================================================================
 *  State advance functions
 * ========================================================================= */

/**
 * @brief Normal-mode LFSR/FSM state advance (hot path).
 *
 * No initialization-mode NLF feedback into A[4] or B[10].
 * Called once per 8-byte keystream output.
 */
static inline void kcipher2_next_normal(KCipher2* c) {
    uint32_t nL1 = sub_k2(c->r2 + c->b[4]);
    uint32_t nR1 = sub_k2(c->l2 + c->b[9]);
    uint32_t nL2 = sub_k2(c->l1);
    uint32_t nR2 = sub_k2(c->r1);

    uint32_t temp1 = ((c->a[0] << 8) & 0xffffff00u) ^ amul0[(c->a[0] >> 24) & 0xff];
    uint32_t nA4 = temp1 ^ c->a[3];

    uint32_t b0_saved = c->b[0];

    if (c->a[2] & 0x40000000u)
        temp1 = ((b0_saved << 8) & 0xffffff00u) ^ amul1[(b0_saved >> 24) & 0xff];
    else
        temp1 = ((b0_saved << 8) & 0xffffff00u) ^ amul2[(b0_saved >> 24) & 0xff];

    uint32_t temp2;
    if (c->a[2] & 0x80000000u)
        temp2 = ((c->b[8] << 8) & 0xffffff00u) ^ amul3[(c->b[8] >> 24) & 0xff];
    else
        temp2 = c->b[8];

    uint32_t nB10 = temp1 ^ c->b[1] ^ c->b[6] ^ temp2;
...
/**
 * @brief Initialization-mode LFSR/FSM state advance.
 *
 * Differs from normal mode: NLF feedback is XORed into both A[4] and B[10].
 * Called exactly 24 times during key setup.
 */
static void kcipher2_next_init(KCipher2* c) {
    uint32_t nL1 = sub_k2(c->r2 + c->b[4]);
    uint32_t nR1 = sub_k2(c->l2 + c->b[9]);
    uint32_t nL2 = sub_k2(c->l1);
    uint32_t nR2 = sub_k2(c->r1);
    ...
    uint32_t nB10 = temp1 ^ c->b[1] ^ c->b[6] ^ temp2;
    nB10 ^= nlf(c->b[10], c->l2, c->l1, c->a[0]);
...
/**
 * @brief Generate one keystream output pair (ZH, ZL) = 8 bytes.
 */
static inline void kcipher2_stream(KCipher2* c, uint32_t* zh, uint32_t* zl) {
    *zh = nlf(c->b[10], c->l2, c->l1, c->a[0]);
    *zl = nlf(c->b[0], c->r2, c->r1, c->a[4]);
}
...
/**
 * @brief Parse 8 hex characters into a uint32_t (big-endian nibble order).
 */
...
/**
 * @brief Initialize KCipher-2 state from hex-encoded key and IV.
 *
 * Performs key expansion (AES-like schedule to produce 12 intermediate
 * words) and 24 initialization-mode rounds.
 *
 * @param c        Caller-allocated state to initialize.
 * @param key_hex  32-character hex string (128-bit key).
 * @param iv_hex   32-character hex string (128-bit IV).
 */
static void kcipher2_init(KCipher2* c, const char* key_hex, const char* iv_hex) {
    uint32_t k[4], iv[4], ik[12];
    memset(c, 0, sizeof(*c));

    for (int i = 0; i < 4; i++)
        hex_to_u32(key_hex + i * 8, &k[i]);
    for (int i = 0; i < 4; i++)
        hex_to_u32(iv_hex + i * 8, &iv[i]);

    /* Key expansion (AES-128-like schedule) */
    ik[0] = k[0];
    ik[1] = k[1];
    ik[2] = k[2];
    ik[3] = k[3];
    ik[4] = ik[0] ^ sub_k2(((ik[3] << 8) & 0xffffff00u) ^ (ik[3] >> 24)) ^ 0x01000000u;
    ik[5] = ik[1] ^ ik[4];
    ik[6] = ik[2] ^ ik[5];
    ik[7] = ik[3] ^ ik[6];
    ik[8] = ik[4] ^ sub_k2((ik[7] << 8) ^ (ik[7] >> 24)) ^ 0x02000000u;
    ik[9] = ik[5] ^ ik[8];
    ik[10] = ik[6] ^ ik[9];
    ik[11] = ik[7] ^ ik[10];

    /* Load expanded key + IV into LFSR registers */
    c->a[0] = ik[4];
    c->a[1] = ik[3];
    c->a[2] = ik[2];
    c->a[3] = ik[1];
    ...
    c->b[9] = ik[5];
    c->b[10] = ik[6];
    c->l1 = c->r1 = c->l2 = c->r2 = 0;

    /* 24 initialization rounds with NLF feedback */
    for (int i = 0; i < 24; i++)
        kcipher2_next_init(c);
}
...
KCipher2* kcipher2_create(const char* key_hex, const char* iv_hex) {
    ...
    KCipher2* ctx = (KCipher2*)calloc(1, sizeof(KCipher2));
    if (!ctx)
        return NULL;

    kcipher2_init(ctx, key_hex, iv_hex);
    return ctx;
}
...
/**
 * @brief Refill the 8-byte keystream buffer when exhausted.
 *
 * Generates the next (ZH, ZL) pair via kcipher2_stream and advances
 * the LFSR state, then serializes the 8 keystream bytes in big-endian.
 */
static void kcipher2_refill(KCipher2* ctx) {
    uint32_t zh, zl;
    kcipher2_stream(ctx, &zh, &zl);
    kcipher2_next_normal(ctx);
    ctx->sbytes[0] = (uint8_t)(zh >> 24);
    ctx->sbytes[1] = (uint8_t)(zh >> 16);
    ctx->sbytes[2] = (uint8_t)(zh >> 8);
    ctx->sbytes[3] = (uint8_t)(zh);
    ctx->sbytes[4] = (uint8_t)(zl >> 24);
    ctx->sbytes[5] = (uint8_t)(zl >> 16);
    ctx->sbytes[6] = (uint8_t)(zl >> 8);
    ctx->sbytes[7] = (uint8_t)(zl);
    ctx->svalid = 8;
}
...
void kcipher2_xor(KCipher2* ctx, uint8_t* data, size_t len) {
    for (size_t i = 0; i < len; i++) {
        if (ctx->svalid == 0)
            kcipher2_refill(ctx);
        data[i] ^= ctx->sbytes[8 - ctx->svalid];
        ctx->svalid--;
    }
}
